...
# OPTIMASI: Cukup gunakan 'eu' untuk menghemat kuota API hingga 50%.
# Pasar Handicap & Over/Under di region EU sudah sangat masif dan lengkap.
ODDS_API_REGIONS = ["eu"]
...
def is_match_now_to_6am(commence_time):
    """
    PERBAIKAN LOGIKA: Mengubah filter waktu mati jam 6 pagi menjadi 
    rentang waktu fleksibel 24 jam ke depan agar match pagi/siang esok hari tetap ter-scan.
    """
    ...
    try:
        if isinstance(commence_time, str):
            ...
            # Menggunakan timezone-aware UTC untuk menghindari bug versi python lama
            match_utc = datetime.fromtimestamp(commence_time, tz=timezone.utc)
    except:
        return False, None

    # Mengubah ke waktu WITA
    match_wita = match_utc + timedelta(hours=WITA_OFFSET)
    now_wita = datetime.now(timezone.utc) + timedelta(hours=WITA_OFFSET)

    # Menyetel batas akhir pemindaian tepat 24 jam dari sekarang
    cutoff_wita = now_wita + timedelta(hours=24)

    # Validasi rentang pertandingan
    is_allowed = now_wita <= match_wita <= cutoff_wita
...
def parse_event(event, source="odds-api"):
    ...
    # PERBAIKAN LOGIKA: Menggunakan format tanggal saja (%Y%m%d) pada match_key.
    # Selisih rilis menit kick-off antar API (misal 07:00 vs 07:05) tidak akan membuat data pecah/gagal merge.
        ...
        if key not in seen_keys:
            ...
            for existing in merged:
                if existing.get("match_key") == key:
                    ...
                    # Bersihkan data ganda di dalam daftar ah & ou hasil merge
...
def main():
    ...
    # Ambil daftar kompetisi dari The Odds API
    ...
    # Tarik data dari OddsPAPI (Khusus regional Asia)
    ...
    # Satukan data dan bersihkan duplikasi antar-API
    ...
    # Batasi maksimal 15 match teratas untuk dikirim ke AI agar tidak terkena token limit / limit teks Telegram
    matches = matches[:15]
    ...
    # 1. Kirim daftar pasaran mentah ke Telegram
    raw_msg = build_raw_market_message(matches, total_sports_scanned)
    send_telegram(raw_msg)

    # 2. Analisis menggunakan AI dan kirim hasilnya ke Telegram
