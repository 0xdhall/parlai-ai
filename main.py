            ...
            if key == "totals":
                ...
                if over and under:
                    if over["price"] < under["price"]:
                        ...
                        picks.append((score, f"Over {over['point']}", over["price"], home, away, league))
                    else:
                        score = int(100 - (under["price"] * 10))
                        picks.append((score, f"Under {under['point']}", under["price"], home, away, league))
            ...
            if key == "spreads":
                for o in outcomes:
                    if 1.55 <= o["price"] <= 2.05:
                        ...
                        picks.append((score, pick, o["price"], home, away, league))
...
def main():
    ...
    all_picks = sorted(all_picks, reverse=True)[:8]

    if not all_picks:
        send_telegram("Tidak ada pick kuat hari ini.")
        return
    ...
    msg += "⚠️ Ini scanner market, bukan jaminan menang. Pakai untuk filter parlay 4–5 leg."
