...
# UPGRADE 1: Buka seluruh region market bandar judi di dunia tanpa terkecuali
...
def fetch_odds_oddspapi():
    ...
    try:
        ...
        # UPGRADE 2: Naikkan limit dari 25 menjadi 100 turnamen bola aktif di seluruh dunia
...
def is_match_now_to_6am(commence_time):
    ...
    # Toleransi mundur 2 jam agar match yang baru mulai tidak hilang
    start_scan_utc = now_utc - timedelta(hours=2)
    
    # Tetap mengunci rentang waktu 12 jam agar tidak bocor ke sore besok
...
def build_raw_market_message(matches, total_leagues):
    ...
    msg += "Target Rentang: 12 Jam Ke Depan (Malam ini s/d Subuh)\n"
...
def main():
    ...
    # 1. Tarik Data dari The Odds API (Gabungan 4 Region Terbesar Dunia)
    for sport in sports:
    ...
    # 2. Tarik Data dari OddsPAPI (Sapu Bersih 100 Turnamen)
    ...
    # UPGRADE 3: HAPUS TOTAL pembatasan array potong (seperti [:45]). 
    # Berapapun match malam ini yang lolos filter 12 jam, akan diproses semuanya!

    if not matches:
        send_telegram("Tidak ada pertandingan dalam 12 jam ke depan (malam ini s/d subuh) yang terdeteksi.")
        return
