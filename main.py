...
# 1. AMBIL KREDENSI DARI GITHUB SECRETS / ENVIRO
...
def fetch_odds_oddspapi():
    ...
    try:
        ...
        tournament_ids = [str(t["id"]) for t in active_tournaments[:40]]
...
def is_match_now_to_6am(commence_time):
    """
    PERBAIKAN TOTAL: Anti-Zonk Filter Waktu.
    Menerima berbagai format waktu, toleran terhadap delay data,
    dan mengunci jendela scan hingga 30 jam penuh ke depan.
    """
    ...
    try:
        if isinstance(commence_time, (int, float)):
            ...
            t_str = commence_time.strip().replace(" ", "T")
            if t_str.endswith("Z"):
        ...
        print(f"Gagal membaca format waktu ({commence_time}): {e}")
        return False, None

    now_utc = datetime.now(timezone.utc)
    
    # Melonggarkan batas mundur (match yang sedang berjalan atau tunda 6 jam lalu tetap lolos)
    start_scan_utc = now_utc - timedelta(hours=6)
    
    # Melonggarkan batas maju hingga 30 jam ke depan agar semua match malam s/d subuh terjaring
    cutoff_utc = now_utc + timedelta(hours=30)
